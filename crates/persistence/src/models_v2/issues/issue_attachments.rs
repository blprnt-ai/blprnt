...
impl IssueAttachmentModel {
  pub async fn migrate(db: &DbConnection) -> Result<()> {
    ...
    db.query(
      format!("DEFINE FIELD IF NOT EXISTS issue ON TABLE {ISSUE_ATTACHMENTS_TABLE} TYPE record<{ISSUES_TABLE}> REFERENCE ON DELETE UNSET;"),
    )
    .await?;

    db.query(
      format!("DEFINE FIELD IF NOT EXISTS actor ON TABLE {ISSUE_ATTACHMENTS_TABLE} TYPE record<{EMPLOYEES_TABLE}> REFERENCE ON DELETE UNSET;"),
    )
    .await?;
