  ...
  U: EventListener + Send + 'static,
{
  ...
  pub fn new(
    ...
    Ok(EventLoop { poll, pty, tx, rx: PeekableReceiver::new(rx), terminal, event_proxy, drain_on_exit, ref_test })
  }

  pub fn channel(&self) -> Arc<EventLoopSender> {
    Arc::new(EventLoopSender { sender: self.tx.clone(), poller: self.poll.clone() })
  }
