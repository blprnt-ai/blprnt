...
use crate::vte::ansi::Hyperlink as VteHyperlink;
...
impl Cell {
  ...
  pub fn set_underline_color(&mut self, color: Option<Color>) {
    ...
    if color.is_none()
      && self.extra.as_ref().is_none_or(|extra| extra.zerowidth.is_empty() && extra.hyperlink.is_none())
    {
      self.extra = None;
    } else {
      let extra = self.extra.get_or_insert(Default::default());
  ...
  pub fn set_hyperlink(&mut self, hyperlink: Option<Hyperlink>) {
    let should_drop = hyperlink.is_none()
      && self.extra.as_ref().is_none_or(|extra| extra.zerowidth.is_empty() && extra.underline_color.is_none());
