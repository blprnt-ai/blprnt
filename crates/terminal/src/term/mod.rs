...
use crate::term::cell::LineLength;
use crate::term::color::Colors;
use crate::vi_mode::ViModeCursor;
use crate::vi_mode::ViMotion;
use crate::vte::ansi::Attr;
use crate::vte::ansi::CharsetIndex;
use crate::vte::ansi::Color;
use crate::vte::ansi::CursorShape;
use crate::vte::ansi::CursorStyle;
use crate::vte::ansi::Handler;
use crate::vte::ansi::Hyperlink;
use crate::vte::ansi::KeyboardModes;
use crate::vte::ansi::KeyboardModesApplyBehavior;
use crate::vte::ansi::NamedColor;
use crate::vte::ansi::NamedMode;
use crate::vte::ansi::NamedPrivateMode;
use crate::vte::ansi::PrivateMode;
use crate::vte::ansi::Rgb;
use crate::vte::ansi::StandardCharset;
use crate::vte::ansi::{self};
...
impl Iterator for TermDamageIterator<'_> {
  ...
  fn next(&mut self) -> Option<Self::Item> {
    self.line_damage.find_map(|line| {
      line.is_damaged().then_some(LineDamageBounds::new(line.line + self.display_offset, line.left, line.right))
    })
  }
}
...
impl TermDamageState {
  fn new(num_cols: usize, num_lines: usize) -> Self {
    let lines = (0..num_lines).map(|line| LineDamageBounds::undamaged(line, num_cols)).collect();
...
impl<T> Term<T> {
    ...
    match self.selection.as_ref() {
      Some(Selection { ty: SelectionType::Block, .. }) => {
        for line in (start.line.0..end.line.0).map(Line::from) {
          res += self.line_to_string(line, start.column..end.column, start.column.0 != 0).trim_end();
          res += "\n";
        }
  ...
  pub fn resize<S: Dimensions>(&mut self, size: S) {
    ...
    self.vi_mode_cursor.point.line = cmp::max(cmp::min(vi_point.line, viewport_bottom), viewport_top);
  ...
  pub fn swap_alt(&mut self) {
    ...
    let keyboard_mode = self.keyboard_mode_stack.last().copied().unwrap_or(KeyboardModes::NO_MODE).into();
  ...
  fn damage_cursor(&mut self) {
    ...
    let point = Point::new(self.grid.cursor.point.line.0 as usize, self.grid.cursor.point.column);
    self.damage.damage_point(point);
  }
...
impl<T: EventListener> Handler for Term<T> {
  ...
  fn report_keyboard_mode(&mut self) {
    ...
    let current_mode = self.keyboard_mode_stack.last().unwrap_or(&KeyboardModes::NO_MODE).bits();
    ...
    match mode {
      ...
      ansi::ClearMode::All => {
        if self.mode.contains(TermMode::ALT_SCREEN) {
          ...
          self.vi_mode_cursor.point.line = (self.vi_mode_cursor.point.line - lines).grid_clamp(self, Boundary::Grid);
        }
      ...
      ansi::ClearMode::Saved if self.history_size() > 0 => {
        ...
        self.vi_mode_cursor.point.line = self.vi_mode_cursor.point.line.grid_clamp(self, Boundary::Cursor);
  ...
  fn report_private_mode(&mut self, mode: PrivateMode) {
    ...
    let state = match mode {
      PrivateMode::Named(mode) => match mode {
        ...
        NamedPrivateMode::ReportMouseClicks => self.mode.contains(TermMode::MOUSE_REPORT_CLICK).into(),
        NamedPrivateMode::ReportCellMouseMotion => self.mode.contains(TermMode::MOUSE_DRAG).into(),
        NamedPrivateMode::ReportAllMouseMotion => self.mode.contains(TermMode::MOUSE_MOTION).into(),
        NamedPrivateMode::ReportFocusInOut => self.mode.contains(TermMode::FOCUS_IN_OUT).into(),
        ...
        NamedPrivateMode::AlternateScroll => self.mode.contains(TermMode::ALTERNATE_SCROLL).into(),
        NamedPrivateMode::UrgencyHints => self.mode.contains(TermMode::URGENCY_HINTS).into(),
        NamedPrivateMode::SwapScreenAndSetRestoreCursor => self.mode.contains(TermMode::ALT_SCREEN).into(),
        NamedPrivateMode::BracketedPaste => self.mode.contains(TermMode::BRACKETED_PASTE).into(),
    ...
    self.event_proxy.send_event(Event::PtyWrite(format!("\x1b[?{};{}$y", mode.raw(), state as u8,)));
  }
  ...
  fn report_mode(&mut self, mode: ansi::Mode) {
    ...
    let state = match mode {
      ansi::Mode::Named(mode) => match mode {
        ...
        NamedMode::LineFeedNewLine => self.mode.contains(TermMode::LINE_FEED_NEW_LINE).into(),
      },
  ...
  fn push_title(&mut self) {
    ...
    if self.title_stack.len() >= TITLE_STACK_MAX_DEPTH {
      let removed = self.title_stack.remove(0);
      trace!("Removing '{removed:?}' from bottom of title stack that exceeds its maximum depth");
    }
...
pub mod test {
  ...
  pub fn mock_term(content: &str) -> Term<VoidListener> {
    ...
    let num_cols =
      lines.iter().map(|line| line.chars().filter(|c| *c != '\r').map(|c| c.width().unwrap()).sum()).max().unwrap_or(0);
...
mod tests {
  ...
  fn simple_selection_works() {
    ...
    term.selection =
      Some(Selection::new(SelectionType::Simple, Point { line: Line(0), column: Column(0) }, Side::Left));
    ...
    // A wrapline.
    term.selection =
      Some(Selection::new(SelectionType::Simple, Point { line: Line(2), column: Column(0) }, Side::Left));
    if let Some(s) = term.selection.as_mut() {
  ...
  fn semantic_selection_works() {
    ...
    {
      term.selection =
        Some(Selection::new(SelectionType::Semantic, Point { line: Line(0), column: Column(1) }, Side::Left));
      assert_eq!(term.selection_to_string(), Some(String::from("aa")));
    }

    {
      term.selection =
        Some(Selection::new(SelectionType::Semantic, Point { line: Line(0), column: Column(4) }, Side::Left));
      assert_eq!(term.selection_to_string(), Some(String::from("aaa")));
    }

    {
      term.selection =
        Some(Selection::new(SelectionType::Semantic, Point { line: Line(1), column: Column(1) }, Side::Left));
      assert_eq!(term.selection_to_string(), Some(String::from("aaa")));
    }
  }
  ...
  fn line_selection_works() {
    ...
    term.selection = Some(Selection::new(SelectionType::Lines, Point { line: Line(0), column: Column(3) }, Side::Left));
  ...
  fn block_selection_works() {
    ...
    term.selection = Some(Selection::new(SelectionType::Block, Point { line: Line(0), column: Column(3) }, Side::Left));
  ...
  fn damage_public_usage() {
    ...
    assert_eq!(damaged_lines.next(), Some(LineDamageBounds { line: display_offset, left: 0, right: 0 }));
    assert_eq!(damaged_lines.next(), Some(LineDamageBounds { line: display_offset + 1, left: 0, right: 0 }));
    assert_eq!(damaged_lines.next(), None);
  }
