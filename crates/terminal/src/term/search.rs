...
use regex_automata::hybrid::dfa::Builder;
use regex_automata::hybrid::dfa::Cache;
use regex_automata::hybrid::dfa::Config;
...
impl RegexSearch {
  ...
  pub fn new(search: &str) -> Result<RegexSearch, Box<BuildError>> {
    ...
    let config = Config::new().minimum_cache_clear_count(Some(3)).minimum_bytes_per_state(Some(10));
    ...
    let left_rdfa =
      LazyDfa::new(search, config.clone(), syntax_config, thompson_config.clone(), Direction::Right, true)?;
    let has_empty = left_rdfa.dfa.get_nfa().has_empty();
    let left_fdfa =
      LazyDfa::new(search, config.clone(), syntax_config, thompson_config.clone(), Direction::Left, has_empty)?;

    // Create DFAs to find start/end in left-to-right search.
    let right_fdfa =
      LazyDfa::new(search, config.clone(), syntax_config, thompson_config.clone(), Direction::Right, has_empty)?;
    let right_rdfa = LazyDfa::new(search, config, syntax_config, thompson_config, Direction::Left, true)?;
...
impl LazyDfa {
  fn new(
    ...
    config = if match_all { config.match_kind(MatchKind::All) } else { config.match_kind(MatchKind::LeftmostFirst) };

    // Create the DFA.
    let dfa = Builder::new().configure(config).syntax(syntax).thompson(thompson).build(search)?;
...
impl<T> Term<T> {
  ...
  fn skip_fullwidth<'a>(&self, iter: &'a mut GridIterator<'_, Cell>, cell: &mut &'a Cell, direction: Direction) {
    match direction {
      ...
      Direction::Right if cell.flags.contains(Flags::WIDE_CHAR) && iter.point().column < self.last_column() => {
        iter.next();
      }
  ...
  pub fn semantic_search_left(&self, point: Point) -> Point {
    match self.inline_search_left(point, self.semantic_escape_chars()) {
      ...
      Ok(point) => {
        ...
        self.grid.iter_from(point).find(|cell| !cell.flags.intersects(wide_spacer)).map_or(point, |cell| cell.point)
      }
      Err(point) => point,
    }
  }
...
impl<'a, T> RegexIter<'a, T> {
  pub fn new(start: Point, end: Point, direction: Direction, term: &'a Term<T>, regex: &'a mut RegexSearch) -> Self {
