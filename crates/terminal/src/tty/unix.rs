...
use rustix_openpty::rustix::termios::InputModes;
#[cfg(any(target_os = "linux", target_os = "macos"))]
use rustix_openpty::rustix::termios::OptionalActions;
use rustix_openpty::rustix::termios::Winsize;
#[cfg(any(target_os = "linux", target_os = "macos"))]
use rustix_openpty::rustix::termios::{self};
use signal_hook::SigId;
use signal_hook::consts as sigconsts;
use signal_hook::low_level::pipe as signal_pipe;
use signal_hook::low_level::unregister as unregister_signal;
...
fn get_pw_entry(buf: &mut [i8; 1024]) -> Result<Passwd<'_>> {
  ...
  let status = unsafe { libc::getpwuid_r(uid, entry.as_mut_ptr(), buf.as_mut_ptr() as *mut _, buf.len(), &mut res) };
...
pub fn from_fd(config: &Options, window_id: u64, master: OwnedFd, slave: OwnedFd) -> Result<Pty> {
  ...
  let working_directory =
    config.working_directory.as_ref().and_then(|path| CString::new(path.as_os_str().as_bytes()).ok());
...
impl EventedReadWrite for Pty {
  ...
  #[inline]
  unsafe fn register(&mut self, poll: &Arc<Poller>, mut interest: Event, poll_opts: PollMode) -> Result<()> {
    interest.key = PTY_READ_WRITE_TOKEN;
    unsafe {
      poll.add_with_mode(&self.file, interest, poll_opts)?;
    }

    unsafe { poll.add_with_mode(&self.signals, Event::readable(PTY_CHILD_EVENT_TOKEN), PollMode::Level) }
  }
  ...
  fn reregister(&mut self, poll: &Arc<Poller>, mut interest: Event, poll_opts: PollMode) -> Result<()> {
    ...
    poll.modify_with_mode(&self.signals, Event::readable(PTY_CHILD_EVENT_TOKEN), PollMode::Level)
  }
