...
impl<R: Read + Send + 'static> UnblockedReader<R> {
  ...
  pub fn new(mut source: R, pipe_capacity: usize) -> Self {
    ...
    let interest = Arc::new(Registration { interest: Mutex::<Option<Interest>>::new(None), end: PipeEnd::Reader });
...
impl<W: Write + Send + 'static> UnblockedWriter<W> {
  ...
  pub fn new(mut sink: W, pipe_capacity: usize) -> Self {
    ...
    let interest = Arc::new(Registration { interest: Mutex::<Option<Interest>>::new(None), end: PipeEnd::Writer });
