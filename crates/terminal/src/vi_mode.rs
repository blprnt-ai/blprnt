  ...
  pub fn motion<T: EventListener>(mut self, term: &mut Term<T>, motion: ViMotion) -> Self {
    match motion {
      ...
      ViMotion::Left => {
        ...
        if self.point.column == 0 && self.point.line > term.topmost_line() && is_wrap(term, wrap_point) {
...
fn first_occupied<T>(term: &Term<T>, mut point: Point) -> Point {
  ...
  let occupied = first_occupied_in_line(term, point.line).unwrap_or_else(|| Point::new(point.line, last_column));
...
fn semantic<T: EventListener>(term: &Term<T>, mut point: Point, direction: Direction, side: Side) -> Point {
...
fn word<T: EventListener>(term: &Term<T>, mut point: Point, direction: Direction, side: Side) -> Point {
  // Make sure we jump above wide chars.
  point = term.expand_wide(point, direction);
...
fn first_occupied_in_line<T>(term: &Term<T>, line: Line) -> Option<Point> {
  (0..term.columns()).map(|col| Point::new(line, Column(col))).find(|&point| !is_space(term, point))
}

/// Find last non-empty cell in line.
fn last_occupied_in_line<T>(term: &Term<T>, line: Line) -> Option<Point> {
  (0..term.columns()).map(|col| Point::new(line, Column(col))).rfind(|&point| !is_space(term, point))
}

/// Advance point based on direction.
fn advance<T>(term: &Term<T>, point: Point, direction: Direction) -> Point {
  if direction == Direction::Left { point.sub(term, Boundary::Grid, 1) } else { point.add(term, Boundary::Grid, 1) }
}
...
fn is_boundary<T>(term: &Term<T>, point: Point, direction: Direction) -> bool {
  (point.line <= term.topmost_line() && point.column == 0 && direction == Direction::Left)
    || (point.line == term.bottommost_line() && point.column + 1 >= term.columns() && direction == Direction::Right)
}
