...
import type { ProviderSheetViewmodel } from './provider-sheet.viewmodel'

interface ProviderSheetProps {
  viewmodel: ProviderSheetViewmodel
}

export const ProviderSheet = observer(({ viewmodel }: ProviderSheetProps) => {
      ...
      >
        <form className="flex h-full flex-col" onSubmit={(event) => void handleSubmit(event)}>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6">
            <ProviderFields provider={viewmodel.editor.provider} showProviderSelector={false} />
          </div>
