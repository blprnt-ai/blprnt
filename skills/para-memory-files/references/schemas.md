...
This reference defines the durable structure used by the `para-memory-files` skill inside blprnt.
...
- employee storage lives under `memories/employees/<employee_id>`
- project storage lives under `memories/projects/<project_id>`

Those concrete paths are not the public contract agents should depend on.
...
- employee scope without `path` writes to `memory/YYYY-MM-DD.md`
