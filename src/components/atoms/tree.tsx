  ...
  const toggleExpanded = useCallback(
    (nodeId: string) => {
      setExpandedIds((prev) => {
        ...
        onExpandedChange?.(Array.from(newSet.values()))
...
export const TreeNodeTrigger = ({ children, className, onClick, ...props }: TreeNodeTriggerProps) => {
  ...
  return (
    <motion.div
      className={cn('h-8 group relative flex cursor-pointer items-center transition-all duration-200', className)}
      role="button"
      style={{ paddingLeft: level * (indent ?? 0) + 6 }}
      onClick={(e) => {
...
export const TreeIcon = ({ icon, hasChildren = false, className, ...props }: TreeIconProps) => {
  ...
  return (
    <motion.span
      className={cn('mr-2 flex h-4.5 w-4.5 items-center justify-center text-muted-foreground', className)}
