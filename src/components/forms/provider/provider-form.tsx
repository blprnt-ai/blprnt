...
interface ProviderFormProps {
  onProviderSaved: (provider: Provider) => void
}

export const ProviderForm = ({ onProviderSaved }: ProviderFormProps) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {viewmodel.provider.isOauthProvider && (
        <div className="flex flex-col gap-2 text-sm text-muted-foreground font-thin">
          You must have the {viewmodel.provider.provider === 'claude_code' ? 'claude-code' : 'codex'} cli installed and
          configured.
        </div>
      )}
      ...
      <div className="flex justify-end">
        <Button
          ...
          onClick={handleSave}
        >
          {viewmodel.isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : verb}
        </Button>
      </div>
    </div>
  )
...
const ProviderCard = ({ title, description, provider, selectedProvider, onSelect }: ProviderCardProps) => {
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 text-sm text-muted-foreground font-thin">{description}</div>
    </div>
  )
}
